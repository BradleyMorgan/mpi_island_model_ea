...
void log_pop_stats(ea &solver, island &isle, MPI_Datatype &visa_type) {
    ...
    if(isle.id==0) {
        ...
        for (auto it = solver.solutions.population.begin(); it !=solver.solutions.population.begin() + 20; ++it) {
            ...
            std::fprintf(config::solpop_out, "%d," "%d," "%s," "%d," "%d," "%s," "%s," "%d," "%d,", solver.run.id, solver.run.eval.id, it->id, it->source, it->locale, it->parents[0], it->parents[1], it->selected, it->survival);
...
void log_fn_topology_stats(ea &solver, ea &meta, topology &t) {
    ...
    LOG(3, 0, 0, " *** STATS (run %d, eval %d): filtered topologies size %lu, mem[0] fit = %f \r\n", meta.run.id, meta.run.eval.id, filtered_top.size(), filtered_top[0].fitness);
    ...
    LOG(3, 0, 0, " *** STATS (run %d, eval %d): best topology fit = %f", meta.run.id, meta.run.eval.id, meta.run.eval.stats.local_best_topo_fitness);
    
    if(filtered_top[0].fitness > meta.run.eval.stats.global_best_topo_fitness || meta.run.eval.stats.global_best_topo_fitness == 0.0) {
        LOG(3, 0, 0, " *** STATS (run %d, eval %d): found new global best topology %f > %f", meta.run.id, meta.run.eval.id, meta.topologies.population[0].fitness, meta.run.eval.stats.global_best_topo_fitness);
        ...
        log_topology_matrix(meta.variant.islands, filtered_top[0], meta.run.eval.stats.topo_best_count);
    }
    ...
    LOG(3, 0, 0, " *** STATS (run %d, eval %d): total topology fit = %f", meta.run.id, meta.run.eval.id, total_topo_fitness);
    ...
    LOG(3, 0, 0, "\r\n%0d,"     "%9d,"     "%10d,"             "%16.11f,"    "%16.11f,"            "%16.11f,",
                 t.id,       t.rounds,   t.channel_count,    t.fitness,     t.total_migration_time, meta.run.eval.stats.total_migrate_time);
    ...
    LOG(3, 0, 0, "\r\n%3.10f,"          "%012d,"               "%014d,"                 "%013d,"                         "%3.10f,"                       "%3.10f,"               "%3.10f,",
                 average_topo_fitness,   filtered_top[0].id,  filtered_top[0].rounds, filtered_top[0].channel_count,  filtered_top[0].round_fitness,  filtered_top[0].fitness, meta.run.eval.stats.local_best_topo_fitness);
...
void log_fn_eval_stats(ea &solver, ea &meta, topology &t) {
    ...
    LOG(3, 0, 0, "STATS (run %d, eval %d): solutions size %lu, mem[0] fit = %f \r\n", solver.run.id, solver.run.eval.id, solver.solutions.population.size(), solver.solutions.population[0].fitness);
    ...
    LOG(3, 0, 0, "STATS (run %d, eval %d): local best = %f, local best fitnesses tracked = %lu\r\n", solver.run.id, solver.run.eval.id, solver.run.eval.stats.local_best_fitness, solver.run.eval.stats.average_local_best_fitnesses.size());
    
    if(filtered_sol[0].fitness >solver.run.eval.stats.global_best_fitness) {
        LOG(3, 0, 0, "STATS (run %d, eval %d): found new global best solution %f > %f\r\n", solver.run.id, solver.run.eval.id, filtered_sol[0].fitness,solver.run.eval.stats.global_best_fitness);
   ...
   solver.run.eval.stats.average_global_best_fitness = std::accumulate(solver.run.eval.stats.average_global_best_fitnesses.begin(),solver.run.eval.stats.average_global_best_fitnesses.end(), 0.0) /solver.run.eval.stats.average_global_best_fitnesses.size();
    ...
    double average_gather_time = solver.run.eval.stats.total_gather_time / solver.run.eval.id;
    double average_scatter_time = solver.run.eval.stats.total_scatter_time / solver.run.eval.id;
    double average_migrate_time = solver.run.eval.stats.total_migrate_time / solver.run.eval.id;
    
    std::fprintf(config::sol_stats_out, "%d," "%d," "%3.10f," "%3.10f," "%3.10f," "%13.10f," "%3.10f,", solver.run.id, solver.run.eval.id,  average_fitness, solver.run.eval.stats.local_best_fitness, solver.run.eval.stats.global_best_fitness, solver.run.eval.stats.average_local_best_fitness, solver.run.eval.stats.average_global_best_fitness);
...
void log_run_stats(ea &meta, objective<topology> &o) {
